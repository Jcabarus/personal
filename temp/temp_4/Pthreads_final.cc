...
//These create the vectors to hold the results of our image function calls that will change the orginal image around by performing operations on the pixels
vector<unsigned char> image_rotation, image_color_change, blurred_image, brightness_change, gray_im, negative, histo;
int width,height,channels,threads;
unsigned char *image;
...
//This function will create a number threads depending on the size of the image
int number_of_threads_needed(int height, int width)
{
    //Will hold the number of threads we need
    int threads;
    //Grabs the total number of pixels in our image
    int pixels=height*width;
    //50000 Pixels are required to get 1 thread
    int pixels_for_thread=250000;
    //Pixels/50000 = number of threads that will be created

    threads=pixels/pixels_for_thread;
...
// void* gray_scale(int height, int width, int channels, unsigned char *image) // Debug
// {
//     vector<unsigned char> gray_image(height*width*channels);
//     int image_index,r,g,b,gray;
...
void *pthread_worker(void *arg)
{
    ...
    //pthread_barrier_wait(&wait_flag);
    // gray=gray_scale(height,width,channels,image); // Debug
    //pthread_barrier_wait(&wait_flag);
    ...
    int image_index,r,g,b,gray;
    vector<unsigned char>im_gray(height*width*channels);
    cout << "Thread created" << endl; // Debug, remove later
    ...
    for(int k=0;k<height;k++)
    {
        for(int l=0;l<width;l++)
        {
            ...
            if(4==channels)
                gray_im[image_index+3]=image[image_index+3];
        }
    }
...
int main(int argc, char * argv[])
{
    ...
    //This function finds the number of threads we need to handle the pixels
    threads=number_of_threads_needed(height, width);

    // int thread_ids=0;
    int thread_ids;
    pthread_t THREADS[threads];
    pthread_barrier_init(&wait_flag, NULL,threads);

    for(int k=0;k<threads;k++)
    {
        pthread_create(&THREADS[k], NULL, pthread_worker, &thread_ids);
        // pthread_create(&THREADS[k], NULL, gray_scale, (void *) thread_ids);
        thread_ids++;
    }
    ...
    //name="Pthreads Image Rotation";
    //save_image(width, height,channels,image_rotation,name);
    //name="Pthreads Color Change";
    //save_image(width, height,channels,image_color_change,name);
    //name="Pthreads Brightness Change";
    //save_image(width, height,channels,brightness_change,name);
    name="Pthreads Gray Scale";
    save_image(width,height,channels,gray_im,name);
    //name="Pthreads Negative-Image";
    //save_image(width,height,channels,negative,name);
    //name="Pthreads Histogram-Image";
